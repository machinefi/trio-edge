...
def make_logits_processors(
    ...
    if repetition_penalty and repetition_penalty != 0.0:
        processors.append(_make_repetition_penalty(repetition_penalty, repetition_context_size))
    return processors
...
def _resolve_cache(
    ...
    if prompt_cache_manager is not None:
        ...
        if cache_hit:
            ...
            prompt_cache = (
                prompt_cache_manager.kv_cache or prompt_cache_manager.get_or_create_cache()
            )
            logger.debug(
                "Cache MISS — deferring for visual similarity check (prefix_match=%s)", prefix_hit
            )
...
def _run_prefill(
    ...
    _sm_active = (
        prompt_cache_manager is not None and prompt_cache_manager.streaming_memory is not None
    )
    ...
    with mx.stream(_generation_stream):
        ...
        if (
            ...
            and (prefill_step_size is None or inputs_embeds.shape[1] <= prefill_step_size)
        ):
        ...
        visual_hit = may_visual_hit and prompt_cache_manager.check_visual_hit(
            inputs_embeds,
            original_input_ids,
        ...
        if _sm_active and not visual_hit:
            ...
            if _streaming_pos_data is not None:
                ...
                prompt_cache_manager._streaming_position_offset = (
                    int(_streaming_pos_data["biased_pos_ids"].max().item()) + 1
                )
                ...
                prompt_cache_manager._streaming_position_offset = (
                    int(lm._position_ids.max().item()) + 1
                )
