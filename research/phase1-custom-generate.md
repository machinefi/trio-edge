...
**Expected gains:** Prefill -25~40%, Decode +10~20% (early stop), Memory -15%
...
class CacheManager:
    ...
    - Visual prefix caching: reuse KV for unchanged visual tokens
    - Cache invalidation: detect when prompt changes
    """
    ...
    def __init__(self, model, max_kv_size=None):
        ...
        self._cached_seq_len = 0    # how many tokens are already in cache
        self._prompt_token_len = 0  # length of text-only prompt (reusable part)

    def get_or_create(self, input_ids, prompt_token_ids=None):
        """Get existing cache or create new one.

        Returns:
            (cache, skip_tokens): cache to use, number of tokens to skip in prefill
        """
        ...
        if self._cache is not None and self._prompt_hash == prompt_hash:
            # Same prompt — reuse cached prompt KV, only prefill new visual tokens
            skip = self._prompt_token_len
            return self._cache, skip
    ...
    def update_seq_len(self, n):
        """Update tracked sequence length after prefill."""
        self._cached_seq_len = n
...
|----------|-------------|---------------|--------|
| Same prompt, same image | Yes | Same | Skip entire prefill, decode only |
| Same prompt, new frame | Yes | Different | Reuse prompt KV, re-prefill visual |
| New prompt | No | Any | Full prefill from scratch |

**Estimated prefill savings:**
- Video monitoring (same prompt, new frame): skip ~50-100 prompt tokens, only
  prefill ~200-400 visual tokens → **30-50% prefill reduction**
- Same image re-query: skip entire prefill → **~95% prefill reduction**
...
def generate_step(
    ...
    **kwargs,
    ...
    for n in range(max_tokens):
        ...
        # Early stop: if max logprob > threshold, likely EOS coming
...
class EarlyStopConfig:
    """Configuration for confidence-based early stopping.
    ...
    confidence_threshold: float = 0.95  # stop if max logprob > this
    eos_confidence: float = 0.8   # stop if EOS token prob > this

    def should_stop(self, token, logprobs, n_generated):
        if n_generated < self.min_tokens:
            return False
        max_prob = mx.exp(logprobs.max()).item()
        return max_prob > self.confidence_threshold
```

**Use case impact:**
- POPE (yes/no): currently generates 10-16 tokens ("Yes, there is a dog in the image.")
  → with early stop: 1-3 tokens ("Yes") → **80% fewer decode steps**
- Open-ended description: no effect (confidence stays low through generation)
...
- [ ] Prefill latency -20%+ on consecutive same-prompt requests
- [ ] Decode tokens reduced 50%+ on POPE (early stop)
- [ ] No memory increase (peak memory ≤ baseline)
- [ ] All existing tests pass
- [ ] Streaming generation works (webcam/screen narrator)
- [ ] No regressions on any of the 6 models
