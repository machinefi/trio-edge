...
def run_perf_bench(engine, model_key, config_name, n_runs=3, max_tokens=64):
    ...
    for i in range(n_runs):
        ...
        print(f"  Run {i+1}: prefill={m.prefill_ms:.0f}ms  "
    ...
    summary = {
        "model": model_key, "config": config_name, "n_runs": n_runs,
...
def run_kv_reuse_bench(backend, model_key, threshold=0.95, n_runs=3,
                       frames_per_seq=5, noise=0.01, max_tokens=32):
    ...
    for run_i in range(n_runs + 1):
        ...
        frames = [base] + [perturb(base, noise, seed=run_i * 100 + j)
                           for j in range(1, frames_per_seq)]
    ...
    with _wired_limit(backend._model):
        ...
        for run_i in range(n_runs):
            ...
            print(f"  Run {run_i+1}: baseline_warm={avg_bl_warm:.0f}ms  "
    ...
    print(f"  Warm:  baseline={summary['baseline_warm_ms']:.0f}ms  "
...
def run_streammem_bench(backend, model_key, n_frames=10, budget=6000,
                        max_tokens=32):
    ...
    summary = {
        "model": model_key, "n_frames": n_frames, "budget": budget,
...
def main():
    ...
    parser.add_argument("--pope-samples", type=int, default=100,
                        help="POPE samples per config (default: 100)")
    parser.add_argument("--eval-runs", type=int, default=3,
                        help="Eval suite runs per case (default: 3)")
    parser.add_argument("--skip-pope", action="store_true",
                        help="Skip POPE accuracy benchmarks")
    parser.add_argument("--skip-eval", action="store_true",
                        help="Skip synthetic eval suite")
        ...
        for config_name, overrides in combos:
            try:
                ...
                print(f"  Loaded in {load_time:.1f}s  "
                      f"backend={health['backend']['backend']}")
                ...
                # StreamMem: special bench (continuous frames)
                    ...
                    if not args.skip_eval:
                        try:
                            perf_summary = run_perf_bench(
                                engine, model_key, config_name,
                                n_runs=args.eval_runs,
                            )
                            results["perf"] = str(OUTPUT_DIR / f"perf_{model_key}_{config_name}.json")
                    ...
                    if not args.skip_pope:
                        try:
                            pope_result = run_pope(
                                engine, model_key, config_name,
                                samples=args.pope_samples,
                            )
                            results["pope"] = str(OUTPUT_DIR / f"pope_{model_key}_{config_name}.json")
                        except Exception as e:
    ...
    for model_key, configs in all_results.items():
        ...
        for config_name, result in configs.items():
            if "error" in result:
                ...
                kv = result["kv_reuse"]
                print(f"    {config_name}: speedup={kv['speedup_warm']:.2f}x  "
                      f"savings={kv['savings_pct']:.1f}%")
            elif "streammem" in result:
                sm = result["streammem"]
                print(f"    {config_name}: first={sm['first_ms']:.0f}ms  "
